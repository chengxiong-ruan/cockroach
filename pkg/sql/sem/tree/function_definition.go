...
func (fd *ResolvedFunctionDefinition) MatchOverload(
	...
	matched := func(ol overloadImpl, schema string) bool {
		return schema == ol.(*QualifiedOverload).Schema && (argTypes == nil || ol.params().Match(argTypes))
	}
...
func (fd *ResolvedFunctionDefinition) GetClass() (FunctionClass, error) {
	ret := fd.Overloads[0].(*QualifiedOverload).Class
	for i := range fd.Overloads {
		if fd.Overloads[i].(*QualifiedOverload).Class != ret {
...
func (fd *ResolvedFunctionDefinition) GetReturnLabel() ([]string, error) {
	ret := fd.Overloads[0].(*QualifiedOverload).ReturnLabels
	for i := range fd.Overloads {
		if len(ret) != len(fd.Overloads[i].(*QualifiedOverload).ReturnLabels) {
...
func (fd *ResolvedFunctionDefinition) GetHasSequenceArguments() (bool, error) {
	ret := fd.Overloads[0].(*QualifiedOverload).HasSequenceArguments
	for i := range fd.Overloads {
		if ret != fd.Overloads[i].(*QualifiedOverload).HasSequenceArguments {
...
func NewResolvedFunctionDefinition(name string, numOverloads int) *ResolvedFunctionDefinition {
	return &ResolvedFunctionDefinition{
		Name:      name,
		Overloads: make([]overloadImpl, 0, numOverloads),
	}
}
